...
export function AppSidebar() {
  ...
  // FIX: initial population from the store no longer depends on the
  // IntersectionObserver/sentinel firing. On mobile, shadcn's Sidebar
  // typically doesn't mount its content into the DOM until the drawer is
  // opened — so sentinelRef.current is null on first effect run, and the
  // observer never gets created, meaning history never populated. This
  // effect runs independently of sidebar mount/open state, so history
  // shows up as soon as initialHistory is available in the store,
  // regardless of whether the sidebar has ever been opened on this device.
  useEffect(() => {
  ...
  // IntersectionObserver now purely handles infinite-scroll pagination.
  // If the sentinel isn't mounted yet (sidebar closed on mobile), this
  // effect re-runs and successfully attaches once the sidebar opens and
  // the sentinel actually exists in the DOM, since sentinelRef.current
  // is re-read on every dependency change.
  useEffect(() => {
